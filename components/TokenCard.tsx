  ...
  return (
    <div className="bg-card rounded-lg border border-gray-800/50 hover:border-primary/50 hover:shadow-lg hover:shadow-primary/10 transition-all duration-200 overflow-hidden relative aspect-square flex flex-col group">
      ...
      <button
        ...
        className="absolute top-2 right-2 z-10 p-1.5 rounded-full bg-background/90 hover:bg-primary/20 border border-gray-800/50 hover:border-primary/50 transition-all"
      ...
      {/* PFP Image Container - Centered Square */}
      <div className="w-full flex items-center justify-center pt-4 pb-3 px-4">
        <div className="w-48 h-48 rounded-lg bg-gray-900/50 flex items-center justify-center overflow-hidden border-2 border-primary/30 group-hover:border-primary/60 transition-colors flex-shrink-0">
          {token.image && token.image !== '/placeholder-pfp.png' ? (
            <img src={token.image} alt={token.name} className="w-full h-full object-cover" />
          ) : (
            <span className="text-7xl font-bold text-gray-600">{token.symbol?.[0] || '?'}</span>
          )}
        </div>
      </div>
      ...
      <div className="px-4 pb-4 flex-1 flex flex-col min-h-0">
        ...
        <div className="mb-3 text-center">
          <h3 className="text-base font-bold text-text truncate leading-tight">{token.name}</h3>
          <p className="text-sm text-gray-400 truncate">${token.symbol}</p>
        </div>

        {/* Stats Grid */}
        <div className="space-y-2 text-xs flex-1 min-h-0">
          ...
          {token.category && (
            <div className="flex justify-between">
              ...
              <span className="text-text font-medium text-right text-xs leading-tight max-w-[60%] truncate" title={token.category}>
                {token.category}
              </span>
            </div>
          )}
        ...
        {/* Social Links & Buttons - Two Rows */}
        <div className="space-y-2 pt-3 mt-3 border-t border-gray-800/50">
          {/* Row 1: Social Links */}
          <div className="flex items-center justify-center space-x-4">
            ...
            <button
              ...
              disabled={dexLoading}
              className="px-2 py-1 text-[10px] font-bold bg-primary/10 text-primary hover:bg-primary hover:text-white border border-primary/50 hover:border-primary rounded transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            <button
              ...
              disabled={dexLoading}
              className="px-2 py-1 text-[10px] font-bold bg-secondary/10 text-secondary hover:bg-secondary hover:text-white border border-secondary/50 hover:border-secondary rounded transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              title="View on Axiom"
            >
              {dexLoading ? '...' : 'AXM'}
            </button>
          ...
          <div className="flex justify-center">
            <button
              ...
              className="flex items-center space-x-1 group hover:bg-gray-800/50 px-2 py-1 rounded transition-all"
              title="Copy address"
            >
